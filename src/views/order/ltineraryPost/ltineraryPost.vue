<template>
  <!--<div>行程单邮寄查询</div>-->
  <div id="TravelMailQuery" class="container">
    <div class="header" ref="header">
      <div class="search-container clearfix">
        ...
        <div class="search-item search-ipt">
          <el-date-picker
            v-model="filter.startFlightDate"
            class="w-180"
            size="medium"
...
<!--        </div>-->
        <div class="search-item search-ipt fl">
          <el-select size="medium" v-model="filter.emailStatus" clearable  placeholder="邮寄状态">
<!--            <el-option label="全部" value="0"></el-option>-->
      ...
      <div class="btn-container">
        <el-button type="success" size="medium" @click="exportMailPostList" :disabled="travelPostList.total === 0">导出</el-button>
      </div>
    ...
    <div class="main">
      ...
      <div class="table-container">
        <el-table
          ...
          <el-table-column v-if="tableList.mailNo" label="快递单号" prop="mailNo" min-width="180"></el-table-column>
          ...
          <el-table-column v-if="tableList.flightDate" label="航班起飞时间" prop="flightDate" min-width="120"></el-table-column>
          <el-table-column v-if="tableList.addressee" label="收件人" prop="addressee" min-width="100"></el-table-column>
          <el-table-column v-if="tableList.phone" label="电话" prop="phone" min-width="120"></el-table-column>
          <el-table-column v-if="tableList.address" label="地址" min-width="240">
            <template slot-scope="scope">
              <div class="inl-block text-over pointer" :title="scope.row.address">{{scope.row.address}}</div>
            </template>
          </el-table-column>
          <el-table-column v-if="tableList.applyDate" label="申请日期" prop="applyDate" min-width="120"></el-table-column>
          <el-table-column v-if="tableList.emailStatus" label="邮寄状态" min-width="100">
            <template slot-scope="scope">
          ...
          <el-table-column v-if="tableList.dcDate" label="邮寄日期" prop="dcDate" min-width="160"></el-table-column>
          <el-table-column v-if="tableList.channel" label="渠道编号" prop="channel" min-width="160">
            <template slot-scope="scope">
              {{scope.row.channel | channelMap}}
            </template>
          </el-table-column>
          <el-table-column v-if="tableList.recoveryStatus" label="是否通知回收" min-width="120">
            <template slot-scope="scope">
...
import { getCurrentDate, excelList, getRouteName } from '../../../utils/index'
import fieldMap from './refer'
import { checkAttrs } from '../../../mixins'
import { computWidth } from '../../../mixins/computWidt'

export default {
  name: 'TravelMailQuery',
  data () {
    return {
      componentName: 'travelMailQuery',
      checkList: fieldMap,
      checkedAll: false,
  ...
  methods: {
    ...
    exportMailPostList () {
      ...
      let name = getRouteName('/travelMailQuery')
      let type = 'xls'
      data.start = 1
