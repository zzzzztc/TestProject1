  ...
  <div class="container">
    ...
    <div class="header" ref="header">
      ...
      <div class="btn-container">
        <el-button size="medium" @click="handleEdit(null)">添加预警配置</el-button>
      </div>
    </div>
    <div class="main">
...
<!--          <el-table-column label="序号" type="index" width="50" fixed="left"></el-table-column>-->
          ...
          <el-table-column label="状态">
            <template slot-scope="scope">
              <span v-if="scope.row.paramStatus === '1'">启用</span>
              <span v-if="scope.row.paramStatus === '0'">禁用</span>
            </template>
          </el-table-column>
        ...
        <div class="dialog-main">
          ...
          <div class="row">
            <div class="column">
              ...
              <div class="ipt-medium">
                <el-input type="textarea" :rows="4" clearable placeholder="参数值" v-model="editForm.paramValue" size="medium"></el-input>
          ...
          <div class="row">
            <div class="column">
              ...
              <div class="ipt-medium">
                <el-select size="medium" v-model="channelModel" multiple>
                  <el-option v-for="item in channelList" :key="item.id" :label="item.chalName" :value="item.chalCode"></el-option>
                </el-select>
                <div class="warning-info" v-if="isEmpty && !channelModel.length">渠道不可为空</div>
              </div>
            </div>
            <div class="column">
              <div class="label required">状态</div>
              <div class="ipt-medium">
                <el-select size="medium" v-model="editForm.paramStatus">
...
import { computeTableHeight, getChannelList } from '../../../utils/index'
export default {
  name: 'WarnDeploy',
  // 该模块与业务参数维护模块功能一致，只是通过字段过滤出预警配置，
  data () {
    return {
      searchForm: {
        isEarlyWarning: 1,
  ...
  methods: {
    ...
    // 弹出框数据 -- 保存
    popupEdit () {
      if (this.editForm.paramCode && this.editForm.paramName && this.editForm.paramValue && this.editForm.remark && this.channelModel.length) {
        let data = JSON.parse(JSON.stringify(this.editForm))
        data.isEarlyWarning = 1 // 该字段为默认字段
        data.channel = this.channelModel.join(',')
        if (this.isReset === false) {
          editBussParamConfig(data).then(() => {
    ...
    dataBind (params) {
      ...
      this.editForm.paramStatus = params.paramStatus
      this.channelModel = params.channel.split(',')
    }
  }
}
</script>
