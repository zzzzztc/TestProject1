<template>
  <!--<div>退票一审</div>-->
  <div id="RefundAudit" class="container">
      ...
      <div class="search-container clearfix">
        ...
        <div class="search-item search-ipt">
          <el-select size="medium" placeholder="退票状态" v-model="filter.ticStatus" clearable>
            <el-option label="退票申请中" value="1"></el-option>
            <el-option label="一审拒绝" value="3"></el-option>
            <el-option label="退票申请失败" value="9"></el-option>
          </el-select>
        </div>
        <div class="search-item search-ipt">
          <el-date-picker
...
<!--          <el-table-column label="序号" type="index" width="50" fixed="left"></el-table-column>-->
          ...
          <el-table-column key="2" label="订单号" v-if="tableList.ordercode" prop="ordercode" min-width="160"></el-table-column>
          <el-table-column key="3" label="PNR" v-if="tableList.pnr" prop="pnr" min-width="100"></el-table-column>
          <el-table-column key="4" label="市场方" v-if="tableList.airways" prop="airways" min-width="90"></el-table-column>
          <el-table-column key="5" label="承运方" v-if="tableList.airline" prop="airline" min-width="90"></el-table-column>
          ...
          <el-table-column key="8" label="旧票号" v-if="tableList.initialTktNo" prop="initialTktNo" min-width="160"></el-table-column>
          <el-table-column key="9" label="票号" v-if="tableList.eticketno" prop="eticketno" min-width="160"></el-table-column>
          <el-table-column key="10" label="旅客姓名" v-if="tableList.name" prop="name" min-width="90"></el-table-column>
          <el-table-column key="11" label="航班号" v-if="tableList.fnumber" prop="fnumber" min-width="90"></el-table-column>
          <el-table-column key="12" label="航意险" v-if="tableList.unexoInsr" prop="unexoInsr" min-width="90"></el-table-column>
          <el-table-column key="13" label="航延险" v-if="tableList.delayInsr" prop="delayInsr" min-width="90"></el-table-column>
          <el-table-column key="14" label="支付金额" v-if="tableList.paymoney" prop="paymoney" min-width="90"></el-table-column>
          <el-table-column key="15" label="附加商品金额" v-if="tableList.productMoney" min-width="120"></el-table-column>
          <el-table-column key="16" label="应退金额" v-if="tableList.actualRefundMoney" prop="actualRefundMoney" min-width="90"></el-table-column>
          <el-table-column key="17" label="支付积分" v-if="tableList.integralFare" prop="integralFare" min-width="90"></el-table-column>
          <el-table-column key="18" label="银行订单号" v-if="tableList.paybillno" prop="paybillno" min-width="180"></el-table-column>
          <el-table-column key="19" label="退票单号" v-if="tableList.refundNo" prop="refundNo" min-width="160"></el-table-column>
          ...
          <el-table-column key="23" label="退票状态" v-if="tableList.tstatus" min-width="120" prop="tstatus"></el-table-column>
          <el-table-column key="30" label="一审拒绝原因" v-if="tableList.firstAuditReason" prop="firstAuditReason" min-width="120"></el-table-column>
          <el-table-column key="29" label="退票证明" v-if="tableList.picUrl" min-width="100">
          ...
          <el-table-column key="25" label="审核人" v-if="tableList.opAccount" prop="opAccount" min-width="120"></el-table-column>
          <el-table-column key="26" label="申请退票时间" v-if="tableList.happendate" prop="happendate" min-width="160"></el-table-column>
          <el-table-column key="27" label="行程单是否需回收" v-if="tableList.printFlag" min-width="140">
            <template slot-scope="scope">
          ...
          <el-table-column key="31" label="疫情隔离险" v-if="tableList.epidemicInsr" prop="epidemicInsr" min-width="140"></el-table-column>
          <el-table-column key="32" label="航空退票险" v-if="tableList.refundTicketInsr" prop="refundTicketInsr" min-width="140"></el-table-column>
          <el-table-column key="28" label="操作" min-width="160" v-if="checked.length">
            <template slot-scope="scope">
              <span class="active-text mr-5" v-if="scope.row.status === '1' || scope.row.status === '3'" @click="switchDialog('first',1, scope.row)">退票审核</span>
              <span class="active-text mr-5" v-else @click="switchDialog('detail',1, scope.row)">查看详情</span>
              <span class="active-text mr-5" @click="switchDialog('first',0, scope.row)">审核记录</span>
    ...
    <!--审核记录-->
    <!--退票审核-->
    <RefundDiaLog ref="refundAby" :changeSelect.sync="changeSelect" @closeParentDialog="closeDialog"></RefundDiaLog>
    <!--同步商品详情 -->
...
export default {
  ...
  data () {
    return {
      ...
      reviewInfo: {
        idPara: '', // id
        refundCharge: '1231', // 退票手续费
        actualRefundMoney: '', // 实际退款
        refundRemark: '', // 备注
        actualRefundIntegral: '', // 退票手续费（积分）
        refundChargeIntegral: '' // 实际退款（积分）
      }, // 退票审核数据
      refundInfo: {}, // 退票拒绝数据
      refundAuditList: {},
      checkList: fieldMap,
  ...
  methods: {
    ...
    exportPayDetail () {
      ...
      let name = getRouteName('/refundAudit')
      let type = 'xls'
